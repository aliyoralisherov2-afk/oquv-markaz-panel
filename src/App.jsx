...
// ================= YANGA QO'SHILGAN KOMPONENT =================

import './App.css'; // CSS faylingiz to'g'ri ulanganiga ishonch hosil qiling

// 1. src/App.jsx faylingizning ichidagi eski LoginScreen'ni shu kodga almashtiring:

function LoginScreen({ onLogin }) {
  // Animatsiya holati uchun state (ro'yxatdan o'tish oynasiga o'tish uchun)
  const [isActive, setIsActive] = useState(false);
  
  // Login inputlari uchun statelar
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // Oddiy login tekshiruvi (buni xohlagancha o'zgartirishingiz mumkin)
  ...
  return (
    // Ekran markaziga tekislash konteyneri
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#0f172a' }}>
      ...
      <div className={`auth-wrapper ${isActive ? 'active' : ''}`}>
        ...
        {/* --- TIZIMGA KIRISH (LOGIN) FORMASI --- */}
        <div className="form-box login">
          <div style={{ textAlign: "center", marginBottom: "30px" }}>
            <h2 style={{ fontSize: 32, fontWeight: 600 }}>Tizimga kirish</h2>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)" }}>Pro Panel boshqaruviga xush kelibsiz</p>
          </div>
          ...
          <form onSubmit={handleLogin}>
            ...
            {error && <p style={{ color: "#ff4d4d", fontSize: "13px", marginBottom: "15px", textAlign: "center" }}>Login yoki parol noto'g'ri!</p>}
        ...
        <div className="form-box register">
          <div style={{ textAlign: "center", marginBottom: "30px" }}>
            <h2 style={{ fontSize: 32, fontWeight: 600 }}>Ro'yxatdan o'tish</h2>
          </div>
        ...
        {/* --- SLAYD BO'LADIGAN CHROYLIK PANEL --- */}
        <div className="toggle-box">
          
          {/* Chap taraf paneli - ro'yxatdan o'tish oynasida ko'rinadi */}
          <div className="toggle-panel toggle-left">
            <h2>Xush kelibsiz!</h2>
            {/* "va" so'zi pastga tushadigan qilib joylangan joyi shu yerda: */}
            <p>Profilingiz bormi? Unda tizimga kiring <br /> va ishlashda davom eting.</p>
            <button className="toggle-btn" onClick={() => setIsActive(false)}>Kirish</button>
          </div>

          {/* O'ng taraf paneli - login oynasida ko'rinadi */}
          <div className="toggle-panel toggle-right">
            <h2>Salom, Do'stim!</h2>
            <p>Hali hisobingiz yo'qmi? Hoziroq ro'yxatdan o'ting va barcha imkoniyatlardan foydalaning.</p>
            <button className="toggle-btn" onClick={() => setIsActive(true)}>Ro'yxatdan o'tish</button>
          </div>
...
export default function App() {
  // YANGA QO'SHILGAN: Avtorizatsiya holatini saqlash
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numParticles; i++) {
      particles.push({
        ...
        color: Math.random() > 0.5 ? "rgba(0, 242, 254, " : "rgba(155, 81, 224, ",
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#000000", color: "#ffffff", fontFamily: "'Segoe UI', system-ui, sans-serif", position: "relative", overflowX: "hidden" }}>
      <style>{`
        input, select, button { font-family: inherit; }
        input:focus, select:focus { border-color: #00f2fe !important; box-shadow: 0 0 10px rgba(0, 242, 254, 0.3) !important; }
      ...
      {/* Orqa fon animatsiyasi barcha sahifalar uchun ishlayveradi */}
      ...
      {!isAuthenticated ? (
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <header style={{ 
            ...
            <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ background: "linear-gradient(135deg, #00f2fe, #9b51e0)", borderRadius: "10px", width: "32px", height: "32px", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 15px rgba(0,242,254,0.4)" }}>
                  <i className="ti ti-school" style={{ fontSize: 18, color: "#000000" }} aria-hidden="true" />
                </div>
                <span style={{ fontWeight: 600, fontSize: 18, letterSpacing: "0.5px", background: "linear-gradient(to right, #ffffff, #a0aec0)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Pro Panel</span>
              </div>
              
              <nav style={{ display: "flex", gap: 4, background: "rgba(255,255,255,0.03)", padding: "4px", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.05)" }}>
                {TABS.map((t, i) => (
                  <button key={t} onClick={() => { setTab(i); setHistoryLesson(null); }}
                    style={{ 
                      padding: "6px 14px", 
                      fontSize: 13, 
          ...
          <main style={{ maxWidth: 900, margin: "0 auto", padding: "2rem 1rem 4rem" }}>
            ...
            {tab === 1 && <NewLessonTab data={data} updateData={updateData} onCreated={() => setTab(2)} setSelectedGroup={setSelectedGroup} setSelectedLesson={setSelectedLesson} />}
            {tab === 2 && <AttendanceTab data={data} updateData={updateData} selectedGroup={selectedGroup} setSelectedGroup={setSelectedGroup} selectedLesson={selectedLesson} setSelectedLesson={setSelectedLesson} />}
            {tab === 3 && <HistoryTab data={data} historyLesson={historyLesson} setHistoryLesson={setHistoryLesson} />}
          </main>
        </div>
      )}
    </div>
...
// ============== QOLGAN QISMLAR O'ZGARISHSZ QOLDI ==============
...
function GroupsTab({ data, updateData }) {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: "1.5rem", letterSpacing: "0.5px" }}>Guruhlar boshqaruvi</h2>

      <div style={{ ...styles.glassCard, padding: "1.25rem", marginBottom: "1.5rem" }}>
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: 10, fontWeight: 500 }}>Yangi guruh qo'shish</p>
        <div style={{ display: "flex", gap: 10 }}>
          <input value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addGroup()}
            placeholder="Guruh nomi (masalan: 1-guruh)" style={{ ...styles.glassInput, flex: 1, fontSize: 14 }} />
          ...
          <div key={g.id} style={{ ...styles.glassCard, overflow: "hidden", transition: "all 0.3s" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", cursor: "pointer" }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <span style={{ fontSize: 12, color: "#9b51e0", background: "rgba(155, 81, 224, 0.12)", border: "1px solid rgba(155, 81, 224, 0.2)", padding: "2px 10px", borderRadius: "20px", fontWeight: 500 }}>{g.students.length} o'quvchi</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {expandedGroup === g.id && (
              <div style={{ borderTop: "1px solid rgba(255, 255, 255, 0.08)", padding: "16px 20px", background: "rgba(0,0,0,0.2)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 10, marginBottom: 14 }}>
                  <input value={newStudentFirst} onChange={(e) => setNewStudentFirst(e.target.value)}
                    placeholder="Ism" style={{ ...styles.glassInput, fontSize: 13 }} />
                  <input value={newStudentLast} onChange={(e) => setNewStudentLast(e.target.value)}
                    onKeyDown={(e) => e.key === "Enter" && addStudent(g.id)}
                    placeholder="Familiya" style={{ ...styles.glassInput, fontSize: 13 }} />
                  <button onClick={() => addStudent(g.id)} disabled={!newStudentFirst.trim() || !newStudentLast.trim()} style={{ ...styles.neonButton, padding: "0 18px", fontSize: 13, background: (!newStudentFirst.trim() || !newStudentLast.trim()) ? "rgba(255,255,255,0.05)" : "linear-gradient(135deg, #9b51e0 0%, #7f00ff 100%)", color: "#fff", cursor: (!newStudentFirst.trim() || !newStudentLast.trim()) ? "not-allowed" : "pointer" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {g.students.map((s, i) => (
                      <div key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px", background: "rgba(255, 255, 255, 0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: "8px" }}>
                        <span style={{ fontSize: 14 }}><span style={{ color: "rgba(255,255,255,0.3)", marginRight: 8, fontSize: 12 }}>{i + 1}.</span>{s.firstName} {s.lastName}</span>
                        <button onClick={() => removeStudent(g.id, s.id)} style={{ padding: "4px 8px", fontSize: 12, color: "rgba(255,255,255,0.4)", border: "none", background: "none", cursor: "pointer", transition: "color 0.2s" }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <input value={topic} onChange={(e) => setTopic(e.target.value)}
              ...
              placeholder="Masalan: JavaScript asoslari yoki Obyektlar" style={{ ...styles.glassInput, fontSize: 14 }} />
          </label>

          <button onClick={create} disabled={isDisabled}
            style={{ 
              ...styles.neonButton,
              padding: "12px 20px", 
              fontSize: 14, 
            ...
            {group.students.map((s, i) => {
              ...
              return (
                <div key={s.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr", gap: 10, padding: "12px 16px", alignItems: "center", borderBottom: i < group.students.length - 1 ? "1px solid rgba(255, 255, 255, 0.05)" : "none", background: i % 2 === 1 ? "rgba(255,255,255,0.01)" : "transparent" }}>
                  <span style={{ fontSize: 14, fontWeight: 500 }}><span style={{ color: "rgba(255,255,255,0.3)", marginRight: 8, fontSize: 12 }}>{i + 1}.</span>{s.firstName} {s.lastName}</span>
                  
                  <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                    <button onClick={() => setAttendance(s.id, "present", true)}
                      style={{ fontSize: 11, padding: "5px 10px", background: att.present === true ? "rgba(46, 213, 115, 0.15)" : "transparent", color: att.present === true ? "#2ed573" : "rgba(255,255,255,0.4)", border: `1px solid ${att.present === true ? "#2ed573" : "rgba(255,255,255,0.1)"}`, borderRadius: "6px", cursor: "pointer", fontWeight: 600, transition: "all 0.15s" }}>
                      Keldi
                    </button>
                    <button onClick={() => setAttendance(s.id, "present", false)}
                      style={{ fontSize: 11, padding: "5px 10px", background: att.present === false ? "rgba(255, 71, 87, 0.15)" : "transparent", color: att.present === false ? "#ff4757" : "rgba(255,255,255,0.4)", border: `1px solid ${att.present === false ? "#ff4757" : "rgba(255,255,255,0.1)"}`, borderRadius: "6px", cursor: "pointer", fontWeight: 600, transition: "all 0.15s" }}>
                      Kelmadi
                    </button>
                  </div>
                  
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <input type="number" min="0" max="10" value={att.homework}
                      onChange={(e) => setAttendance(s.id, "homework", e.target.value)}
                      placeholder="—" style={{ ...styles.glassInput, width: 56, textAlign: "center", fontSize: 14, padding: "6px" }} />
                  </div>
                  
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <input type="number" min="0" max="10" value={att.classwork}
                      onChange={(e) => setAttendance(s.id, "classwork", e.target.value)}
                      placeholder="—" style={{ ...styles.glassInput, width: 56, textAlign: "center", fontSize: 14, padding: "6px" }} />
                  </div>
                </div>
              );
  ...
  if (historyLesson) {
    ...
    return (
      <div>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: "1.5rem" }}>{lesson?.date} · <span style={{ color: "#9b51e0" }}>{group?.name}</span></p>
          ...
          <div key={groupId} style={{ marginBottom: "2rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[...lessons].sort((a, b) => b.date.localeCompare(a.date)).map((l) => {
                ...
                return (
                  <div key={l.id} onClick={() => setHistoryLesson(l.id)}
                    ...
                    onMouseLeave={(e) => {
                      ...
                      e.currentTarget.style.background = "rgba(20, 20, 25, 0.65)";
                    }}>
                    <div>
