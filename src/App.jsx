...
const AUTH_KEY     = "pro_panel_auth_v1";

// Hardcoded hisob ma'lumotlari (Login.jsx bilan mos)
// Login.jsx ichida ham xuddi shu qiymatlar tekshiriladi —
// bu yerda faqat sessionStorage tokenini yaratish uchun kerak.
...
export default function App() {
  // Sahifa yuklanganda sessionStorage dan auth holatini o'qiymiz
  ...
  // Ma'lumotlar o'zgarganda localStorage ga yozamiz
  useEffect(() => saveData(data), [data]);

  // ──────────────────────────────────────────
  // Canvas yulduz zarralari animatsiyasi
  // Faqat dashboard ko'rsatilganda ishga tushadi
  // ──────────────────────────────────────────
  useEffect(() => {
    if (!isAuthenticated) return; // Login sahifasida canvas kerak emas
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        ...
        const sz = p.radius * k * 1.5;

        if (px >= 0 && px <= width && py >= 0 && py <= height) {
          const alpha = Math.min(1, (1 - p.z / width) * 0.8);
          ctx.beginPath();
          ctx.arc(px, py, Math.abs(sz), 0, Math.PI * 2);
  ...
  }, [isAuthenticated]); // isAuthenticated o'zgarganda qayta ishga tushadi

  // ──────────────────────────────────────────
  // Kirish / Chiqish handlerlari
  // ──────────────────────────────────────────
  const handleLogin = () => {
    setAuthState(true);
  ...
  // ──────────────────────────────────────────
  // Render: foydalanuvchi kirmaganda — Login
  // ──────────────────────────────────────────
  if (!isAuthenticated) {
  ...
  // ──────────────────────────────────────────
  // Render: autentifikatsiya o'tgan — Dashboard
  // ──────────────────────────────────────────
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <main style={{ maxWidth: 900, margin: "0 auto", padding: "2rem 1rem 4rem" }}>
          {tab === 0 && (
            <GroupsTab data={data} updateData={updateData} />
          )}
          {tab === 1 && (
            <NewLessonTab
              data={data}
              ...
              <div style={{ borderTop: "1px solid rgba(255, 255, 255, 0.08)", padding: "16px 20px", background: "rgba(0,0,0,0.2)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 10, marginBottom: 14 }}>
                  <input
                    ...
                    placeholder="Ism"
                    style={{ ...styles.glassInput, fontSize: 13 }}
                  />
                  <input
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: "0 18px",
                      fontSize: 13,
                      background: (!newStudentFirst.trim() || !newStudentLast.trim()) ? "rgba(255,255,255,0.05)" : "linear-gradient(135deg, #9b51e0 0%, #7f00ff 100%)",
                      color: "#fff",
                      cursor: (!newStudentFirst.trim() || !newStudentLast.trim()) ? "not-allowed" : "pointer",
                    }}
                  >
                ...
                {g.students.length === 0 ? (
                  <p style={{ fontSize: 13, color: "rgba(255,255,255,0.3)", textAlign: "center", padding: "1.5rem 0" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {g.students.map((s, i) => (
                      <div
                        key={s.id}
                        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px", background: "rgba(255, 255, 255, 0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: "8px" }}
                      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <select
              ...
              style={{ ...styles.glassInput, fontSize: 14, cursor: "pointer" }}
            >
              <option value="">— Guruh —</option>
              {data.groups.map((g) => (
                <option key={g.id} value={g.id}>
                  {g.name} ({g.students.length} o'quvchi)
                </option>
              ))}
            </select>
          </label>
          ...
          >
            <i className="ti ti-calendar-plus" aria-hidden="true" style={{ marginRight: 6 }} />
            Davomatga o'tish
          </button>
        </div>
      </div>
...
function AttendanceTab({ data, updateData, selectedGroup, setSelectedGroup, selectedLesson, setSelectedLesson }) {
  ...
  const getAtt = (studentId) =>
    lesson?.attendance?.[studentId] || { present: null, homework: "", classwork: "" };
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: "1.5rem" }}>
        <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <select
            value={selectedGroup || ""}
            onChange={(e) => { setSelectedGroup(Number(e.target.value)); setSelectedLesson(null); }}
            style={{ ...styles.glassInput, fontSize: 14 }}
          >
      ...
      {!group || !lesson ? (
        ...
        <div style={{ ...styles.glassCard, textAlign: "center", padding: "3rem", color: "rgba(255,255,255,0.4)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {group.students.map((s, i) => {
              ...
              return (
                <div
                  key={s.id}
                  style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr", gap: 10, padding: "12px 16px", alignItems: "center", borderBottom: i < group.students.length - 1 ? "1px solid rgba(255, 255, 255, 0.05)" : "none", background: i % 2 === 1 ? "rgba(255,255,255,0.01)" : "transparent" }}
                >
                  ...
                  <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                    <button
                      onClick={() => setAttendance(s.id, "present", true)}
                    ...
                    <button
                      onClick={() => setAttendance(s.id, "present", false)}
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <input
                      type="number" min="0" max="10" value={att.homework}
                      onChange={(e) => setAttendance(s.id, "homework", e.target.value)}
                      placeholder="—"
                      style={{ ...styles.glassInput, width: 56, textAlign: "center", fontSize: 14, padding: "6px" }}
                    />
                  </div>

                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <input
                      type="number" min="0" max="10" value={att.classwork}
...
function HistoryTab({ data, historyLesson, setHistoryLesson }) {
  if (historyLesson) {
    ...
    return (
      <div>
        <button
          onClick={() => setHistoryLesson(null)}
          style={{ fontSize: 13, marginBottom: "1.25rem", display: "flex", alignItems: "center", gap: 6, color: "#00f2fe", background: "none", border: "none", cursor: "pointer", padding: 0, fontWeight: 500 }}
        >
        ...
        {!group || group.students.length === 0 ? (
          ...
          <div style={{ ...styles.glassCard, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255, 255, 255, 0.08)", display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 10, fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.5)", textTransform: "uppercase" }}>
              <span>O'quvchi</span>
              <span style={{ textAlign: "center" }}>Davomat</span>
              <span style={{ textAlign: "center" }}>Vazifa</span>
              <span style={{ textAlign: "center" }}>Ishtirok</span>
            </div>
            {group.students.map((s, i) => {
              ...
              return (
                <div
                  key={s.id}
                  style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 10, padding: "12px 16px", alignItems: "center", borderBottom: i < group.students.length - 1 ? "1px solid rgba(255, 255, 255, 0.05)" : "none", background: i % 2 === 1 ? "rgba(255,255,255,0.01)" : "transparent" }}
                >
                  <span style={{ fontSize: 14 }}>
                    <span style={{ color: "rgba(255,255,255,0.3)", marginRight: 8, fontSize: 12 }}>{i + 1}.</span>
                    {s.firstName} {s.lastName}
                  ...
                  <div style={{ textAlign: "center" }}>
                    {att.present === true
                      ? <span style={{ fontSize: 12, padding: "4px 10px", background: "rgba(46, 213, 115, 0.15)", color: "#2ed573", borderRadius: "6px", fontWeight: 500 }}>Keldi</span>
                      : att.present === false
                        ? <span style={{ fontSize: 12, padding: "4px 10px", background: "rgba(255, 71, 87, 0.15)", color: "#ff4757", borderRadius: "6px", fontWeight: 500 }}>Kelmadi</span>
  ...
  // Guruhlarni id bo'yicha guruhlash
  const grouped = {};
        ...
        Object.entries(grouped).map(([groupId, lessons]) => {
          ...
          return (
            <div key={groupId} style={{ marginBottom: "2rem" }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: "#00f2fe", marginBottom: 10, letterSpacing: "0.5px", textTransform: "uppercase" }}>
                {groupName}
              </p>
